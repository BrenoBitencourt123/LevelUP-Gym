...
function runScenario(label, setValues, lastLoad) {
    const log = {
        workoutId: "w1",
    ...
    const progressionState = lastLoad === null
        ? {}
        : {
            [baseMeta.id]: {
                exerciseId: baseMeta.id,
...
runScenario("Cenario 1: 3x10 (sobe carga)", [
    { reps: 10, load: 80, rir: 1 },
    { reps: 10, load: 80, rir: 1 },
    { reps: 10, load: 80, rir: 2 },
], 80);
runScenario("Cenario 2: 8/8/7 (mantem)", [
    { reps: 8, load: 80, rir: 1 },
    { reps: 8, load: 80, rir: 1 },
    { reps: 7, load: 80, rir: 2 },
], 80);
runScenario("Cenario 3: 5/5/6 (reduz)", [
