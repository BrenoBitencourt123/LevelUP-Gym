...
const StartWorkoutButton = () => {
  ...
  if (isRest) {
    return (
      <button
        onClick={handleClick}
        className="cta-button group bg-secondary/80 hover:bg-secondary"
      >
  ...
  if (isCompletedThisWeek) {
    return (
      <div className="space-y-2">
        <div className="cta-button group bg-primary/15 border-primary/30 cursor-default">
          <div className="w-8 h-8 rounded-lg bg-primary/30 flex items-center justify-center">
            <CheckCircle className="w-4 h-4 text-primary" />
          </div>
          <span className="text-foreground">
            {workout?.titulo || "Treino"} concluído por hoje.
          </span>
        </div>
  ...
  // Treino normal
  return (
    <button onClick={handleClick} className="cta-button cta-primary group">
